jQuery(function ($) {
    ...
    function generate_existing_note(zindex, top, left, noteClass, noteId, groupId, text, image_url, groupcolor, note_type, term) {


        switch (note_type) {
        case "i":
            addImgeNote(noteClass, text, image_url, {
                x: left,
                y: top,
                zind: zindex,
                nId: noteId,
            ...
            break;
        case "v":
            addVideoNote(noteClass, text, image_url, term, {
                x: left,
                y: top,
                zind: zindex,
                nId: noteId,
            ...
            break;
        case "l":
            addLinkNote(noteClass, text, image_url, term, {
                x: left,
                y: top,
                zind: zindex,
                nId: noteId,
            ...
            break;
        case "t":
            addTwitterNote(noteClass, text, term, {
                x: left,
                y: top,
                zind: zindex,
                nId: noteId,
    ...
    function addVideoNote(size, text, url, screenshot, extra) {

        url = url.replace("watch?v=", "v/");
        var nId, x, y, zind, gId, gColor;
        console.log("----------------" + extra.zind);
        ...
        $(area).append(
            '<div class="note note' + size + ' last" noteId=' + nId + ' group_id="' + gId + '" ' +
            'style=" top:' + y + 'px; left:' + x + 'px;z-index: ' + zind + ';"  note_type="v">' +
            '<div class="buttons">' +
            ...
            '<div class="footer-front" style="background-color:' + gColor + ';"></div>' +
            '</div>' +
            '<div class="back">' +
            '<div class="content-back">' +
            '<textarea class="edit" style=position:relative >' + text + '</textarea>' +
            '</div>' +
            '<div class="footer-back" style="background-color:' + gColor + ';"></div>' +
            '</div>' +
            '</div>');
        ...
        if (!extra.x) {
            ...
            if (flag55 == true) {
                $(".last").remove();
                flag55 = false;
                addVideoNote(size, text, url, screenshot, extra);
            } else {
    ...
    function addImgeNote(size, text, url, extra) {
        ...
        $(area).append(
            '<div class="note note' + size + ' last" noteId=' + nId + ' group_id="' + gId + '" ' +
            'style=" top:' + y + 'px; left:' + x + 'px;z-index: ' + zind + ';"  note_type="i">' +
            '<div class="buttons">' +
            '<div class="minus-btn hidden"></div>' +
            '<div class="more-btn"></div>' +
            '<div class="plus-btn"></div>' +
        ...
        if (!extra.x)
        {
            ...
            if (flag55 == true) {
                $(".last").remove();
                flag55 = false;
                addImgeNote(size, text, url, extra);
            } else {
                console.log('note :' + nId + ':' + tries);
                tries = 0;
                $("#area > .note").removeClass("last");
    ...
    function addLinkNote(size, text, url, title, extra) {
        ...
        $(area).append(
            '<div class="note note' + size + ' last" noteId=' + nId + ' group_id="' + gId + '" ' +
            'style=" top:' + y + 'px; left:' + x + 'px;z-index: ' + zind + ';"  note_type="l">' +
            '<div class="buttons">' +
            '<div class="minus-btn hidden"></div>' +
            '<div class="more-btn"></div>' +
            '<div class="plus-btn"></div>' +
            ...
            // '<div class="front">' +
            // '<div class="content-front" style="font-size:15px"><a href="' + url + '" target="_blank">' + title + '</a></div>' +
            // '<div class="footer-fornt" style="background-color:' + gColor + ';"></div>' +
            // '</div>' +
            // '<div class="back">' +
            // '<div class="content-back">' +
            // '<textarea class="edit" style=position:relative >' + text + '</textarea>' +
        ...
        if (!extra.x) 
        {
            ...
            if (flag55 == true) {
                $(".last").remove();
                flag55 = false;
                addLinkNote(size, text, url, title, extra);
            } else {
                console.log('note :' + nId + ':' + tries);
                tries = 0;
                $("#area > .note").removeClass("last");
    ...
    function addTwitterNote(size, text, hashtag, extra) {
        ...
        $(area).append(
            '<div class="note note' + size + ' last" noteId=' + nId + ' group_id="' + gId + '" ' +
            'style=" top:' + y + 'px; left:' + x + 'px;z-index: ' + zind + ';"  note_type="t">' +
            '<div class="buttons">' +
            '<div class="minus-btn hidden"></div>' +
            '<div class="more-btn"></div>' +
            '<div class="plus-btn"></div>' +
        ...
        if (!extra.x) {
            ...
            if (flag55 == true) {
                $(".last").remove();
                flag55 = false;
                addTwitterNote(size, text, hashtag, extra);
            } else {
                console.log('note :' + nId + ':' + tries);
                tries = 0;
                $("#area > .note").removeClass("last");
    ...
    function saveGroups() {
        ...
        if (!user_id) {
            ...
            $('.note').each(function (index, data) {
                ...
                var note_url, note_term;
                switch ($(this).attr("note_type")) {
                case "i":
                    note_url = $(this).find(".content-front").css("background-image").replace('"', '').replace('url(', '').replace(')', '');
                    note_term = "";
                    break;
                case "v":
                    note_url = $(this).find("a").attr("href");
                    note_term = $(this).find(".content-front").css("background-image").replace('"', '').replace('url(', '').replace(')', '');
                    break;
                case "t":
                    note_url = "";
                    note_term = $(this).find(".content-front").text();
                    break;
                case "l":
                    note_url = $(this).find("a").attr("href");
                    note_term = $(this).find("a").text();
                    break;
                }
                ...
                base2.push({
                    ...
                    class: $(this).attr('class').split(' ')[1].replace("note", ""),
                    term: note_term
                });
            });
        ...
        if (!user_id) {
            ...
            base2.once("value", function (snapshot) {
                snapshot.forEach(function (childSnapshot) {
                    ...
                    generate_existing_note(childData.zindex, childData.top, childData.left, childData.class, childData.id, childData.group, childData.description, childData.url, childData.groupcolor, childData.note_type, childData.term);
